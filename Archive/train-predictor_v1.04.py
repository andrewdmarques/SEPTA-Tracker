...
train_ids_to_remove = df[(df['time_to_poi'] > 30) | (df['poi_dist'] > 8)]['train_id'].unique()

# Filter the dataset to remove rows with those train_ids
filtered_df = df[~df['train_id'].isin(train_ids_to_remove)]

# If you want to save it to a new CSV file
filtered_df.to_csv('toi5.csv', index=False)
