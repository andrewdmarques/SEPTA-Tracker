...
file_in = 'toi.csv' # File that has the data to be trained and/or predicted with
...
def add_last_train_info_and_heading_average_optimized(toi2):
    ...
    toi3['last3_train_heading'].fillna(toi3['train_heading'], inplace=True)
    toi3['last3_poi_dist'].fillna(toi3['poi_dist'], inplace=True)
    toi3['last3_date_time'].fillna(toi3['date_time'], inplace=True)
    toi3['last3_train_speed'].fillna(toi3['train_speed'], inplace=True)

    toi3['last4_train_heading'].fillna(toi3['train_heading'], inplace=True)  # Added for last4
    toi3['last4_poi_dist'].fillna(toi3['poi_dist'], inplace=True)  # Added for last4
    toi3['last4_date_time'].fillna(toi3['date_time'], inplace=True)  # Added for last4
    toi3['last4_train_speed'].fillna(toi3['train_speed'], inplace=True)  # Added for last4
...
if model_train: # If the linear regression is to be trained by the data.
    ...
    print('saved lat and lon to csv file')
    ...
    print('correctly read in the csv file and assigned the variables')
...
if model_predict == True and model_train == False:
    ...
    with open(file_model, 'rb') as model_file:
        ...
        # Convert to DataFrame for easier filtering and manipulation
        results_df = pd.DataFrame({'train_id': toi7['train_id'],
