...
dir_csv = os.path.join(dir_database, "Data_CSV_Subset")
...
def create_pdf_plot():
    if os.path.exists(toi_file):
        ...
        scatter = plt.scatter(df['lon'], df['lat'], c=df['train_speed'], cmap='viridis', edgecolors='none')
        plt.colorbar(scatter, label='Train Speed (mph)')
        plt.xlabel('Longitude')
        plt.ylabel('Latitude')
        plt.title('Train Speed Visualization')

        current_time = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        pdf_filename = f"train_speed_plot_{current_time}.pdf"
