...
file_in = 'toi_2024-10-18.csv' # File that has the data to be trained and/or predicted with
...
if model_predict == True and model_train == False:
    ...
    with open(file_model, 'rb') as model_file:
        ...
        # Step 4: Extract relevant columns for the CSV
        date = lowest_predicted_row['date']
        time = pd.to_datetime(lowest_predicted_row['time'], format='%H:%M:%S')
        train_id = lowest_predicted_row['train_id']
        lat = lowest_predicted_row['lat']
        lon = lowest_predicted_row['lon']
        predicted_value = lowest_predicted_row['predicted']

        # Step 5: Print the 'train_id' and 'predicted' value
        print(f"Train ID: {train_id}, Predicted Value: {predicted_value}")

        # Step 6: Calculate 'predicted_arrival_time' by adding the predicted minutes to the 'time'
        predicted_arrival_time = time + timedelta(minutes=predicted_value)

        # Step 7: Create a new DataFrame with the required columns
        output_df = pd.DataFrame({
            ...
            'predicted_arrival_time': [predicted_arrival_time.strftime('%H:%M:%S')]
        })

        # Step 8: Save to CSV
        output_df.to_csv('next-train.csv', index=False)
